...
import { __dirname, __public, commonEsBuildConfig, log, prepareHtml } from "./common.mjs";
...
await writeFile(join(__dirname, "build-meta.json"), JSON.stringify(buildResult.metafile));
...
const buildPrefix = relative(normalize(join(__dirname, "./")), normalize(join(__dirname, "dist")));
const generatedFiles = Object.keys(buildResult.metafile.outputs).map((f) => f.replace(buildPrefix, ""));
await writeFile(join(__dirname, "dist/index.html"), await prepareHtml(generatedFiles));
...
for (const file of readdirSync(__public)) {
    ...
    await copyFile(join(__public, file), join(__dirname, "dist", basename(file)));
}
